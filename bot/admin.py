...
async def process_remove_country(callback_query: types.CallbackQuery):
    ...
    if await is_country_balance_positive(country_id):
        await callback_query.message.answer(
            "Нельзя удалить страну с положительным балансом")
        return
...
async def add_template(message: types.Message, state: FSMContext):
    ...
    keyboard = types.InlineKeyboardMarkup()
    for country in countries:
        button_add_template = types.InlineKeyboardButton(f"{country.flag} {country.name}",
...
async def remove_template(message: types.Message):
    ...
    keyboard = types.InlineKeyboardMarkup()
    for country in countries:
        button_country = types.InlineKeyboardButton(f{country.flag} {country.name}",
...
async def add_bk_country(message: types.Message, state: FSMContext):
    ...
    keyboard = types.InlineKeyboardMarkup()
    for country in countries:
        button_add_bk = types.InlineKeyboardButton(f"{country.flag} {country.name}",
...
async def process_add_bk_by_all_info(message: types.Message,
    ...
    if profile_name.lower() == "отмена":
        ...
        await state.finish()
        await message.answer(
            f"Профиль БК {template.name} ({profile_name}) успешно добавлен")
...
async def edit_bk_country(message: types.Message):
    ...
    keyboard = types.InlineKeyboardMarkup()
    for country in countries:
        button_edit_bk = types.InlineKeyboardButton(f"{country.flag} {country.name}",
...
@admin_required
async def edit_bk_by_template(callback_query: types.CallbackQuery,
                              state: FSMContext):
    """Выбор шаблона для редактирования БК"""
    country_id = int(callback_query.data.split('_|_')[-1])
    if not await get_country_by_id(country_id):
        await callback_query.answer("Ошибка, страна не найдена")
        return
    ...
    keyboard = types.InlineKeyboardMarkup()
    for template in templates:
        button_edit_bk = types.InlineKeyboardButton(template.name,
                                                    callback_data=f"edit_bk_by_template_|_{template.id}")
        keyboard.add(button_edit_bk)
    cancel_button = types.InlineKeyboardButton("Отмена",
                                               callback_data="cancel_editing_bk")
    keyboard.add(cancel_button)
    await callback_query.message.answer(
        "Выберите шаблон для редактирования БК",
        reply_markup=keyboard)
...
@admin_required
async def process_edit_bk_by_template(callback_query: types.CallbackQuery,
                                      state: FSMContext):
    """Обработка выбора шаблона для редактирования БК"""
    template_id = int(callback_query.data.split('_|_')[-1])
    if not await get_template_by_id(template_id):
        await callback_query.answer("Ошибка, шаблон не найден")
        return
    ...
    bks = await get_bk_by_template_id(template_id)
    if not bks:
        await callback_query.message.answer(
            "Нет профилей БК для этого шаблона")
        return
    keyboard = types.InlineKeyboardMarkup()
    for bk in bks:
        button_edit_bk = types.InlineKeyboardButton(
            f"{bk.name} | {round(bk.get_balance(), 3)}",
...
async def process_remove_wallet(callback_query: types.CallbackQuery):
    ...
    if await is_wallet_balance_positive(wallet_id):
        await callback_query.message.answer(
            "Нельзя удалить кошелек с положительным балансом")
        return
    wallet = await get_wallet_by_id(wallet_id)
    await remove_wallet_from_db(wallet_id)
...
async def process_edit_balans_wallet(message: types.Message,
    ...
    await state.finish()
    await add_to_history(message.from_user.id, "wallet",
                         f"Баланс кошелька '{wallet.name}' изменён на '{adjustment}' пользователем {message.from_user.username}")
...
async def process_transfer_country(callback_query: types.CallbackQuery,
    ...
    if data.get("second_variant") == "wallet":
        ...
        templates = await get_templates_by_country_id(data.get("country_id"))
        if not templates:
            await callback_query.message.answer(
                "Нет шаблонов для этой страны, выберите другую")
            return
        keyboard = types.InlineKeyboardMarkup()
        for template in templates:
            button_template = types.InlineKeyboardButton(template.name,
                                                         callback_data=f"transfer_template_{template.id}")
            keyboard.add(button_template)
        await callback_query.message.answer("Выберите шаблон",
                                            reply_markup=keyboard)
        await TransferMoneyState.waiting_for_second_variant_template_id.set()
...
@admin_required
async def process_transfer_template(callback_query: types.CallbackQuery,
                                    state: FSMContext):
    ...
    if not bks:
        await callback_query.message.answer(
            "Нет БК для этого шаблона, выберите другой")
        return
    keyboard = types.InlineKeyboardMarkup()
    for bk in bks:
        button_bk = types.InlineKeyboardButton(bk.name,
                                               callback_data=f"transfer_wallet_{bk.id}")
        keyboard.add(button_bk)
    await callback_query.message.answer("Выберите БК",
                                        reply_markup=keyboard)
    await TransferMoneyState.waiting_for_second_variant_id.set()
...
async def process_transfer_received_sum(message: types.Message,
    ...
    ans_text = f"Переведено {sum_received} с {source_entity} '{source_name}' на {target_entity} '{target_name}' пользователем {message.from_user.username}"
    ...
    if ans:
        ...
        await add_to_commission_history(
            ...
            commission_type=data.get("action"),
            commission_description=(
                f"Комиссия {commission} "
                f"при {'пополнении' if replenish else 'переводе'} "
                f"с {source_entity} '{source_name}' на {target_entity} '{target_name}' "
...
def register_wallets_handlers(dp):
    ...
    dp.register_message_handler(edit_wallet_country,
                                lambda
...
def register_bk_management_handlers(dp):
    ...
    dp.register_callback_query_handler(edit_bk_by_template,
                                       lambda c: c.data.startswith(
                                           'edit_bk_by_country'))
    dp.register_callback_query_handler(cancel_editing_bk, lambda
        c: c.data == 'cancel_editing_bk', state="*")
    dp.register_callback_query_handler(process_edit_bk_by_template,
                                       lambda c: c.data.startswith(
                                           'edit_bk_by_template'))
    ...
    dp.register_callback_query_handler(transfer_money_from_bk,
                                       ...
                                       state=EditBkState.waiting_for_action)
    dp.register_callback_query_handler(process_transfer_template,
                                       lambda c: c.data.startswith(
                                           'transfer_template'),
                                       state=TransferMoneyState.waiting_for_second_variant_template_id)
